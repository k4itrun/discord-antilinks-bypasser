...
const main = (): void => {
    ...
    readline.setPrompt(
        `${COLORS.reset}[${COLORS.red}${COLORS.lightGreen}${COLORS.lightBlue}C${COLORS.reset}${COLORS.lightGreen}L${COLORS.lightBlue}I${COLORS.reset}] Enter a link to process (e.g., discord.gg/vanity): ${COLORS.lightPurple}`
    );

    readline.prompt();
    readline.on('line', url => 
        wrapUrl(url.trim(), readline)
    );
};
